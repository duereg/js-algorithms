...
function traverse(text, node, seps, str_list, ret) {
  for (const t in node.transition) {
    ...
    cchild = traverse(text, s, seps, str_list, cchild);
...
class SuffixTree {
  ...
  toJson() {
    ...
    return traverse(this.text, this.root, this.seps, this.str_list, ret);
  }
}
